...
def extract_info(resume_text):
    ...
    input_text = f"以下は職務経歴書です:\n{text}\n\n" \
                 "このテキストを日本語で要約してください。このテキストに載っている情報以外を含めてはいけません。必ず以下の指示に従ってください。：\n" \
                 "・必ず50文字以内で彼の経歴からわかる実績や強みや使用言語、ツールなどのスキルを書く。" \
                 "・この文章はスカウトや採用担当への紹介に使われるため、彼が何をアピールしているかを推測し必ずそこに触れる。" \
                 "・テキストを見たとき目を引くような魅力的な文章にする。" \
                 "・TOEICの点数は900以上の時にしか記載してはいけない。\n" \
                 "・上限文字数に必ず収めてください。" \
                 "・最後に事実のみが含まれていること、上記の指示に従っていることを確実に確認して日本語で生成してください:"
